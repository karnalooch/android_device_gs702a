...
status_t OMXCameraAdapter::stopAutoFocus()
{
    ...
    LOG_FUNCTION_NAME;
	{
        ...
        if(!mIternalRecordingHint && mParameters3A.Focus == OMX_IMAGE_FocusControlAuto)
        {
...
status_t OMXCameraAdapter::setTouchFocus()
{
    ...
    if ( NO_ERROR == ret )
    {
        ...
        if (regionSize > 1)
        {
            dataLen = sizeof(focusRegionControl)+sizeof(OMX_MANUALFOCUSRECTTYPE)*(regionSize-1);
            ...
            for ( unsigned int n = 0; n < regionSize; n++)
            {
                ...
                CAMHAL_LOGDB("Focus area %d : top = %d left = %d width = %d height = %d ",
